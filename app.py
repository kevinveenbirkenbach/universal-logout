...
def logout():
    ...
    for name in cookie_names:
        ...
        response.set_cookie(key=name, value='', expires=0, max_age=0,domain=parent_domain, path='/', secure=True, httponly=True)
        response.set_cookie(key=name, value='', expires=0, max_age=0,domain=parent_domain, path='/', secure=True, httponly=True, samesite='None')

        # Delete cookie on the exact host
        response.set_cookie(key=name, value='', expires=0, max_age=0,domain=host, path='/', secure=True, httponly=True)
        response.set_cookie(key=name, value='', expires=0, max_age=0,domain=host, path='/', secure=True, httponly=True, samesite='None')

        # Delete host-only cookie (no Domain attribute)
        response.set_cookie(key=name, value='', expires=0, max_age=0, path='/', secure=True, httponly=True)
        response.set_cookie(key=name, value='', expires=0, max_age=0, path='/', secure=True, httponly=True, samesite='None')
